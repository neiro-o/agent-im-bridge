  ...
  "en-US": {
    translation: {
      ...
      client: {
        ...
        helpColumnGroup: "Group",
        helpColumnCommand: "Command",
        helpColumnAliases: "Aliases",
        helpColumnDescription: "Description",
        helpGroupBridge: "Bridge",
        helpGroupAutomation: "Automation",
        helpGroupLocal: "Local control",
        helpGroupAgent: "Pi",
        helpCopyHint: "Copy a command from the table and send it in this chat.",
        helpNew: "Start a new agent session; optionally choose and remember a working directory.",
        helpCompact: "Compact the current context; Pi accepts optional custom instructions.",
        helpStop: "Stop the active agent run.",
        helpStatus: "Show the current agent session status.",
        helpModel: "List available models or switch models.",
        helpScheduleRun: "Run a scheduled task once now.",
        helpScheduleHere: "Bind this chat as a scheduled task's result destination.",
        helpQueueHere: "Bind this chat as a queue's result destination.",
        helpHelp: "Show this command table.",
        helpAgent: "Switch this chat back to Agent mode.",
        helpSsh: "Enter the authorized remote Shell mode.",
        helpUpload: "Upload subsequent Feishu attachments into the current Shell directory.",
        helpUploadCancel: "Stop waiting for Shell-mode uploads.",
        helpDownload: "Download a file, directory, or glob from the Shell workspace.",
        helpMessage:
          "Available commands:\n\n- `/new [path]` (`/n [path]`) - Start a new agent session; optionally start it in a specific directory, e.g. `/new /path/to/project`. A directory given once is remembered and reused by later `/new` without a path\n- `/compact` (`/c`) - Compact the current session context\n- `/stop` (`/s`) - Stop the active agent run\n- `/status` (`/st`) - Show the current agent session status\n- `/model` (`/m`) - List available models, or switch with `/model provider/modelId`\n- `/effort [level]` (`/thinking [level]`) - Show or switch the current model's thinking effort\n- `/agent` - Switch this chat back to Agent mode\n- `/ssh` - Enter authorized SSH mode (when local control is enabled)\n- `/schedule-run <task-name>` - Run a scheduled task once now (the result is sent to the task's target chat)\n- `/schedule-here <task-name>` - Bind this chat as a task's result destination and set the task's owning channel (send this in the chat that should receive the results; an already-bound task must be unbound first)\n- `/queue-here <queue-name>` - Bind this chat as a queue's result destination (send this in the chat that should receive the results; an already-bound queue must be unbound by editing its file with AI)\n- `/help` (`/h`) - Show this help message",
      ...
      gateway: {
        ...
        noActiveSessionForAgentCommand:
          "No active agent session for command `{{command}}`. Send a normal message or use `/new` first.",
        agentCommandUnsupported: "The current agent does not support command `{{command}}`.",
        agentCommandFailed: "Command `{{command}}` failed: {{detail}}",
      },
      cli: {
        examplePrompt:
  ...
  "zh-CN": {
    translation: {
      ...
      client: {
        ...
        helpTitle: "可用命令",
        helpColumnGroup: "分组",
        helpColumnCommand: "命令",
        helpColumnAliases: "别名",
        helpColumnDescription: "说明",
        helpGroupBridge: "Bridge",
        helpGroupAutomation: "自动化",
        helpGroupLocal: "本地控制",
        helpGroupAgent: "Pi",
        helpCopyHint: "从表格复制命令，然后在当前聊天中发送。",
        helpNew: "开始新 Agent 会话；可选指定并记住工作目录。",
        helpCompact: "压缩当前上下文；Pi 支持附加自定义压缩指令。",
        helpStop: "停止当前正在运行的 Agent 任务。",
        helpStatus: "查看当前 Agent 会话状态。",
        helpModel: "查看可用模型或切换模型。",
        helpScheduleRun: "立即运行一次定时任务。",
        helpScheduleHere: "将当前聊天绑定为定时任务的结果目标。",
        helpQueueHere: "将当前聊天绑定为队列的结果目标。",
        helpHelp: "显示此命令表格。",
        helpAgent: "将当前聊天切回 Agent 模式。",
        helpSsh: "进入已授权的远程 Shell 模式。",
        helpUpload: "将后续飞书附件上传到当前 Shell 目录。",
        helpUploadCancel: "取消等待 Shell 模式上传。",
        helpDownload: "从 Shell 工作区下载文件、目录或 glob。",
        helpMessage:
          "可用命令：\n\n- `/new [path]` (`/n [path]`) - 开始一个新会话；可选指定工作目录，例如 `/new /path/to/project`。指定过的目录会被记住，之后不带路径的 `/new` 会继续使用它\n- `/compact` (`/c`) - 压缩当前会话上下文\n- `/stop` (`/s`) - 停止当前正在运行的任务\n- `/status` (`/st`) - 查看当前智能体会话状态\n- `/model` (`/m`) - 查看可用模型，或使用 `/model provider/modelId` 切换模型\n- `/effort [等级]` (`/thinking [等级]`) - 查看或切换当前模型的思考等级\n- `/agent` - 将本聊天切回 Agent 模式\n- `/ssh` - 进入已授权的 SSH 模式（需启用本地控制）\n- `/schedule-run <任务名>` - 立即运行一次定时任务（结果会发送到该任务的目标聊天）\n- `/schedule-here <任务名>` - 把本会话设为该任务结果的发送目标并确定其归属 channel（请在希望接收结果的聊天里发送；已绑定的任务需先解绑）\n- `/queue-here <队列名>` - 把本会话设为队列结果的发送目标（请在希望接收结果的聊天里发送；已绑定的队列需编辑文件解绑）\n- `/help` (`/h`) - 查看这条帮助信息",
      ...
      gateway: {
        ...
        noActiveSessionForAgentCommand:
          "命令 `{{command}}` 需要活动的 Agent 会话。请先发送普通消息或使用 `/new`。",
        agentCommandUnsupported: "当前 Agent 不支持命令 `{{command}}`。",
        agentCommandFailed: "命令 `{{command}}` 执行失败：{{detail}}",
      },
      cli: {
